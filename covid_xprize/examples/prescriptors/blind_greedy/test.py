...
class XPrizePredictor(object):
    ...
    def predict(self,
                full_start_date_str: str,
                start_date_str: str,
                end_date_str: str,
                path_to_ips_file: str,
                old_preds_file=None,
                old_ips_file=None) -> pd.DataFrame:
        # Load the npis into a DataFrame, handling regions
        npis_df = self.load_original_data(path_to_ips_file)
        old_preds_df = self.load_original_data(old_preds_file) if old_preds_file is not None else None
        old_ips_df = self.load_original_data(old_ips_file) if old_ips_file is not None else None
        return self.predict_from_df(full_start_date_str, start_date_str, end_date_str, npis_df, old_preds_df, old_ips_df)
    ...
    def predict_from_df(self,
        ...
        truncated_df = self.df[self.df.Date < full_start_date_str]
       #truncated_df = self.df[self.df.Date < start_date_str]

        if old_preds_df is not None and old_ips_df is not None:
            # Merge the 2 DataFrames
            old_df = old_preds_df.merge(old_ips_df, on=['GeoID', 'Date'], how='left', suffixes=('', '_y'))
            # print(old_df.Date.min())
            # print(old_df.Date.max())
            last_known_date = truncated_df['Date'].max()
            # print(last_known_date)
            # print(start_date)
            old_df_trunc = old_df[(old_df.Date >= full_start_date_str) & (old_df.Date < start_date)]
            # print(old_df_trunc.shape)            
            if len(old_df_trunc) > 0:
                print("Reusing generated data.")
                columns = CONTEXT_COLUMNS + NPI_COLUMNS
                old_df_trunc = old_df_trunc[columns]
                truncated_df = pd.concat([truncated_df, old_df_trunc], ignore_index=True)
        ...
        for g in geos:
            ...
            if len(cdf) == 0:
                ...
                pred_new_cases, preds = self._get_new_cases_preds(cdf, g, npis_gdf, country_samples)

            # Append forecast data to results to return
            initial_total_cases = 0
            if len(cdf) != 0:
            ...
            for i, pred in enumerate(pred_new_cases):
                ...
                forecast["PredictionRatio"].append(preds[i])
                forecast["ConfirmedCases"].append(pred + initial_total_cases)
                initial_total_cases += pred
