...
PREDICT_MODULE = ROOT_DIR / 'predict.py'
...
def add_geo_id(df):
    df["GeoID"] = np.where((df["RegionName"].isnull()) | (df["RegionName"] == ""),
                                      df["CountryName"],
...
def get_predictions(full_start_str, start_date_str, end_date_str, pres_df, past_preds_df=None, past_ips_df=None, countries=None):
    ...
    with tempfile.NamedTemporaryFile() as tmp_ips_file:
        ...
        with tempfile.NamedTemporaryFile() as tmp_pred_file:
            if past_preds_df is not None and past_ips_df is not None:
                with tempfile.NamedTemporaryFile() as tmp_old_preds_file:
                    past_preds_df.to_csv(tmp_old_preds_file.name)

                    with tempfile.NamedTemporaryFile() as tmp_old_ips_file:
                        past_ips_df.to_csv(tmp_old_ips_file.name)
                        try:
                            # Run script to generate predictions
                            output_str = subprocess.check_output(
                                [
                                    'python', PREDICT_MODULE,
                                    '--full_start_date', full_start_str,
                                    '--start_date', start_date_str,
                                    '--end_date', end_date_str,
                                    '--interventions_plan', tmp_ips_file.name,
                                    '--old_predictions', tmp_old_preds_file.name,
                                    '--old_ips', tmp_old_ips_file.name,
