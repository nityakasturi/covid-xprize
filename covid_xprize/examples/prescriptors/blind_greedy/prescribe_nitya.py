...
def prescribe(start_date_str: str,
    ...
    max_weights = weights_df.max(axis=0)[2:]
    ...
    for country_name in hist_df['CountryName'].unique():
        ...
        for region_name in country_df['RegionName'].unique():
            ...
            for date in pd.date_range(start_date, end_date):
                ...
                # Use curr_ips for all dates for this prescription
                for prescription_idx in range(1, NUM_PRESCRIPTIONS - 1):
                    ...
                    for ip in sorted_ips:
                        curr_ips[ip] += (((1 - (geo_weights_df[ip].values[0]/max_weights[ip])) / 8) * IP_MAX_VALUES[ip])
                        prescription_dict[ip].append(min(round(curr_ips[ip]), IP_MAX_VALUES[ip]))
                        stringency += min(round(curr_ips[ip]), IP_MAX_VALUES[ip]) * geo_weights_df[ip].values[0]
                    print(stringency)
            
            # Use curr_ips for all dates for this prescription
            for date in pd.date_range(start_date, end_date):
                prescription_dict['PrescriptionIndex'].append(9)
                prescription_dict['CountryName'].append(country_name)
                prescription_dict['RegionName'].append(region_name)
                prescription_dict['Date'].append(date.strftime("%Y-%m-%d"))
                for ip in IP_COLS:
