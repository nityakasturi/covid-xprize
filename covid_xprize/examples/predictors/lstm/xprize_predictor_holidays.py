...
HOLIDAYS_CONTEXT = os.path.join(DATA_PATH, "holidays.csv")
HOLIDAYS_PRECOMPUTED_CONTEXT = os.path.join(DATA_PATH, "holidays_computed.csv")
...
class XPrizePredictor(object):
    ...
    def __init__(self, path_to_model_weights, data_url):
        if path_to_model_weights:
            ...
            # Load model weights
            nb_context = 2  # Only time series of new cases rate is used as context
        ...
        self.country_samples = self._create_country_samples(self.df, geos, path_to_model_weights)
    ...
    def _load_original_data(data_url):
        ...
        hol_df = pd.read_csv(HOLIDAYS_CONTEXT, parse_dates=['date'],
                                encoding="ISO-8859-1",
                                error_bad_lines=False)
        hol_df["Date"] = hol_df["date"]
        hol_pre_df = pd.read_csv(HOLIDAYS_PRECOMPUTED_CONTEXT, parse_dates=['Date'],
                                encoding="ISO-8859-1",
                                dtype={"RegionName": str},
                                error_bad_lines=False)
        ...
        df_done = df_done.merge(hol_pre_df, on=["CountryName", "RegionName", "Date"])
        if len(df_to_do) == 0:
            return df_done
        df_to_do['HasHoliday'] = df_to_do.apply(lambda row: _normalize_row(row, hol_df), axis=1)
        return pd.concat([df_done, df_to_do])
    ...
    @staticmethod
    def _create_country_samples(df: pd.DataFrame, geos: list, path_to_model_weights) -> dict:
        """
        ...
        """
        context_columns = ['PredictionRatio', 'HasHoliday']
        date_column = 'Date'
        ...
        for g in geos:
            cdf = df[df.GeoID == g]
            # Drop countries with no holiday data
            if not path_to_model_weights:
                cdf.dropna(subset=['HasHoliday'], inplace=True)
            else:
                cdf["HasHoliday"] = cdf.HasHoliday.fillna(0)
            ...
            if len(outcome_samples) > 0:
                ...
                X_context = np.squeeze(X_context)
    ...
    def train(self):
        ...
        country_samples = self._create_country_samples(self.df, geos, None)
    ...
    def _most_affected_geos(df, nb_geos, min_historical_days):
        ...
        new_df = df.copy()
        new_df.dropna(subset=['HasHoliday'], inplace=True)
        gdf = new_df.groupby('GeoID')['PredictionRatio'].agg(['max', 'count']).sort_values(by='max', ascending=False)
