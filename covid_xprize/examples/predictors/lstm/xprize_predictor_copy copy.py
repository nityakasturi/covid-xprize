...
class XPrizePredictor(object):
    ...
    def predict(self,
        ...
        for g in geos:
            if os.path.exists(self.path_to_model_weights + "/" + g + '.h5'):
                self.predictor.load_weights(self.path_to_model_weights + "/" + g + '.h5')
            elif os.path.exists(self.path_to_model_weights + "/" + g.split('-')[0] + '.h5'):
                print("Using " + g.split('-')[0] + " for " + g)
                self.predictor.load_weights(self.path_to_model_weights + "/" + g.split('-')[0] + '.h5')
            else:
                print("Using fallback for " + g)
                self.predictor.load_weights(self.path_to_model_weights + "/" + 'fallback.h5')
    ...
    def train(self):
        ...
        all_geos = self.df.GeoID.unique()
        country_samples = self._create_country_samples(self.df, all_geos)
        geos = self._most_affected_geos(self.df, MAX_NB_COUNTRIES, NB_LOOKBACK_DAYS)
        fallback_country_samples = self._create_country_samples(self.df, geos)
        print("Numpy arrays created")

        # Aggregate data for training
        all_X_context_list = [fallback_country_samples[c]['X_train_context']
                              for c in fallback_country_samples]
        all_X_action_list = [fallback_country_samples[c]['X_train_action']
                             for c in fallback_country_samples]
        all_y_list = [fallback_country_samples[c]['y_train']
                      for c in fallback_country_samples]
        ...
        test_all_X_context_list = [fallback_country_samples[g]['X_test_context']
                                   for g in geos]
        test_all_X_action_list = [fallback_country_samples[g]['X_test_action']
                                  for g in geos]
        test_all_y_list = [fallback_country_samples[g]['y_test']
                           for g in geos]
        ...
        for g in all_geos:
            print('Geo', g)
            X_g_context, X_g_action, g_y = self._permute_data(country_samples[g]['X_train_context'], country_samples[g]['X_train_action'], country_samples[g]['y_train'], seed=1)
            model, training_model = self._construct_model(nb_context=X_g_context.shape[-1],
                                                          nb_action=X_g_action.shape[-1],
                                                          lstm_size=LSTM_SIZE,
                                                          nb_lookback_days=NB_LOOKBACK_DAYS)
            history = self._train_model(training_model, X_g_context, X_g_action, g_y, epochs=2500, verbose=0)
            ...
            test_loss = training_model.evaluate([country_samples[g]['X_test_context'], country_samples[g]['X_test_action']], [country_samples[g]['y_test']])
        ...
        print("FALLBACK")
        X_context, X_action, y = self._permute_data(X_context, X_action, y, seed=1)
